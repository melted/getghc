...
function compute-sha {
    ...
    $stream = [System.IO.File]::Open($abspath, "open", "read")
    $sb=New-Object System.Text.StringBuilder
    $sha1.ComputeHash($stream) | %{ $x=$sb.AppendFormat("{0:x2}", $_) }
    $stream.Close()
    $sb.ToString().ToUpper()
}
...
"@
    ...
    .\msys\usr\bin\bash -l -c "$current_posix/ghc.sh"
}
